...
@Component("RaportPdfGeneratorWarehouse")
public class RaportPdfGeneratorWarehouse {
    ...
    private void getDbData(PdfPTable table, DateSet dates) {
        ...
        for(OrderItemsDto item : orderItems){
            warehouseNames.put(item.getWarehouseId(), item.getWarehouseName());
        }
        ...
        for (OrderDto order : orders) {
            for(OrderItemsDto orderItem : order.getOrderItems()){
                if(!raportMap.containsKey(orderItem.getWarehouseId())){
                    raportMap.put(orderItem.getWarehouseId(), new RaportProd(orderItem.getWarehouseId(), orderItem.getQuantity(), orderItem.getPrice()*orderItem.getQuantity()));
                }else{
                    int quantity = raportMap.get(orderItem.getWarehouseId()).getQuantity();
                    double price = raportMap.get(orderItem.getWarehouseId()).getSumPrice();
                    raportMap.put(orderItem.getWarehouseId(), new RaportProd(orderItem.getWarehouseId(), quantity + orderItem.getQuantity(), orderItem.getQuantity() * orderItem.getPrice() + price));
        ...
        for (RaportProd value : raportMap.values()) {
            table.addCell(warehouseNames.get(value.getProdID()));
